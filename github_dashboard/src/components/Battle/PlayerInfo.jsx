...
function PlayerInfo({ result, setResult, battleResult, setTotalScore, totalScore, reposInfo, setReposStars, reposStars, startBattle, playerInfo, setPlayerName, cleanPlayerInfo, cleanNotExist
}) {
    ...
    useEffect(() => {
        let count = 0;
        reposInfo.length && reposInfo.map((item) => {
    ...
    useEffect(() => {
        if (battleResult !== undefined) {
            battleResult ? dispatch(setResult('Winner')) : dispatch(setResult('Loser'))
        }
    }, [battleResult]);

    return (
        <>
            {Object.keys(result).length ? <h3>{result}</h3> : null}
